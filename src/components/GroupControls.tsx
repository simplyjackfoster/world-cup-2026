  ...
  return (
    <div className="bg-pitch border border-slate-800 rounded-2xl p-4 shadow-card flex flex-col gap-2">
      ...
      <p className="text-xs text-slate-400">
        ...
        {draftKingsLoading && <span className="block text-[11px] text-slate-400 mt-1">Loading DraftKings outrights…</span>}
        ...
        {!draftKingsLoading && draftKingsAsOf && (
          <span className="block text-[11px] text-slate-500 mt-1">DraftKings odds as of {draftKingsAsOf}</span>
        )}
      </p>
    </div>
  );
